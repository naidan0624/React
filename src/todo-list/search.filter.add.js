...
export default function ToDoList() {
  ...
  const [toDoItems, setToDoItems] = useState({ original: [], filtered: [] });
  ...
  const onFormSubmit = (e, todoId) => {
    ...
    if (inputValue.length === 0) {
      ...
      // if in edit mode
        ...
        setToDoItems((currState) => {
          return {
            original: currState.original.map((saveItem, saveItemIdx) => {
              return saveItemIdx === editItemId
                ? { ...saveItem, title: inputValue }
                : saveItem;
            }),
            filtered: currState.filtered.map((saveItem, saveItemIdx) => {
              return saveItemIdx === editItemId
                ? { ...saveItem, title: inputValue }
                : saveItem;
            }),
        ...
        // setToDoItems((currState) => {
        //   return currState.original.map((saveItem, saveItemIdx) => {
  ...
  useEffect(()=>{
    setToDoItems((prevData)=>{
        return {
            ...prevData,
            filtered : prevData.original.filter((toDoItem)=>{
                if (
                    toDoItem.title.includes(searchInputValue)
                ) return toDoItem
            })
        }
    })
  ...
  // const completeList= (toDoIdx)=>{
  //   setToDoItems((currState)=>{
  //     return currState.filter((com,comIdx)=>{
  //       return comIdx === currToDoIdx
  //       return currState
  //     })
  //   })
  // }
  ...
  const readFromLocalStorage = () => {
    ...
    if (Array.isArray(parsedSavedTodos)) setToDoItems(parsedSavedTodos);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("my-to-do-list", JSON.stringify(toDoItems));
  }, [toDoItems]);
  ...
  return (
    <div className="">
        ...
        <label>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setSearchInputValue(e.target.value); //search deer bichigdej bgaa valuegar haruuljin
            }}
          />
        </label>
      ...
      <div>
        ...
        {toDoItems.filtered.map((toDo, toDoIdx) => {
          return (
            <div
            ...
            >
              ...
              {editItemId !== toDoIdx ? ( // do now show "Edit", "Delete" and "Complete" buttons if item is in edit mode
                <div>
                  <button onClick={() => onEdit(toDo, toDoIdx)}>Edit</button>
                  <button onClick={() => onDelete(toDoIdx)}>Delete</button>
                  <button onClick={() => onComplete(toDoIdx)}>Complete</button>
                </div>
              ) : (
